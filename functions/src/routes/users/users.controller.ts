...
async function _updateUserInFirestore(deviceId: any, userId: string) {
    console.log(userId)
    try {
        admin.firestore().collection("employees").doc(userId).update({
...
async function _authenticate(deviceId: string, userId: string, snipeId: string) {
    ...
    if (device['status_label']['id'] !== '1') await DevicesController().checkinDevice(snipeId,  device['id'])

    const checkoutResult = await DevicesController().checkoutDevice(snipeId, device)
    console.log(checkoutResult);
    if (!checkoutResult) return false;

    const updateResult = await DevicesController().updateDeviceInFirestore(deviceId, userId);
...
async function _logOut(deviceId: string, userId: string, snipeId: string) {
    ...
    const checkInResult = await DevicesController().checkinDevice(snipeId, deviceId);
    console.log(checkInResult);
    if (!checkInResult) return false;

    const updateResult = await DevicesController().updateDeviceInFirestore(deviceId, null);
    console.log(updateResult);
    if (!updateResult) return false;
...
class UsersController {
    ...
    async customAuth(req: any, res: Response) {
        ...
        if (!userId || !deviceId || !snipeId || req.method !== 'POST') {
            res.json({ "asdf":"didnt have all yo shit" });
        }
        const valid = await _authenticate(deviceId, userId, snipeId);
        if (!valid) {
            res.json({"401": "failed _authenticate"}) // Invalid username/password
        }
